...
async function fetchMaintainers(octokit: Octokit, owner: string, repo: string): Promise<Set<string>> {
  ...
  try {
    ...
    const maintainers = new Set(
      collaborators
        .filter((collab: { permissions?: { push?: boolean; admin?: boolean } }) => 
  ...
  return getCachedOrFetch(cacheKey, async () => {
    ...
    try {
      while (hasMore && page <= MAX_PAGES) {
        ...
        const filteredPRs = data.items.filter((pr: { created_at: string }) => 
          isAfter(parseISO(pr.created_at), filterDate)
        );
        ...
        pullRequests.push(...filteredPRs.map((pr: { number: number; title: string; state: string; created_at: string; pull_request?: { merged_at?: string }; html_url: string; user: { login: string } }) => ({
          number: pr.number,
          title: pr.title,
          state: pr.state,
          created_at: pr.created_at,
    ...
    const repositories: { [key: string]: { totalPRs: number; mergedPRs: number; openPRs: number; closedPRs: number } } = {};
    const totalStats = {
      totalPRs: 0,
      mergedPRs: 0,
      openPRs: 0,
